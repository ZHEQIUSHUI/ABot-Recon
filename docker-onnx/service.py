...
# torch-free: the "model" is the exported ONNX graph (mounted; ABOT_MODEL_ID = .onnx path
# or a dir containing one). Hot-load / idle-unload machinery unchanged.
MODELS = {"ABot-Recon-ONNX": os.environ.get("ABOT_MODEL_ID", "/app/model/abot_recon.onnx")}
DEFAULT_MODEL = "ABot-Recon-ONNX"

import sys
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))

app = FastAPI(title="ABot-Recon ONNX 建图服务(torch-free)")
...
class ModelManager:
    ...
    def gpu_mem(self):
        return None, None   # torch-free (ORT); GPU mem not tracked
